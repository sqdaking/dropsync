...
// ─── REVISE action ─────────────────────────────────────────────────────────────
// LEAN REVISE — price + stock only, always fetches fresh from Amazon.
// Never touches title, description, images, or aspects.
// Handles all listing types:
//   • Multi-ASIN + multi-var (Color×Size, each combo = own ASIN)
//   • Single ASIN + multi-var (one parent, sizes/colors as children)
//   • Multi-ASIN no var (each color = own ASIN, no size dimension)
//   • Simple single ASIN no variations

async function handleRevise({ body, res, sleep, getEbayUrls }) {
  const {
    access_token, ebaySku: rawSku, sourceUrl, ebayListingId,
    markup: markupRaw, handlingCost: handlingRaw, quantity: qtyRaw,
    // Fallback combo data — used ONLY if Amazon is completely unreachable
    fallbackComboAsin, fallbackComboInStock, fallbackComboPrices,
    fallbackVariations, fallbackPrimaryDimName, fallbackSecondaryDimName,
    fallbackPrice, fallbackInStock,
  } = body;

  if (!access_token || !rawSku || !sourceUrl) {
    return res.status(400).json({ error: 'Missing access_token, ebaySku, or sourceUrl' });
  }

  // Normalise variant SKU → group SKU
  const ebaySku   = rawSku.replace(/^(DS-\d{10,14}-[A-Z0-9]{4,8})-[A-Z0-9_]+$/, '$1');
  const EBAY_API  = getEbayUrls().EBAY_API;
  const markupPct = parseFloat(markupRaw ?? 23);
  const handling  = parseFloat(handlingRaw ?? 2);
  ...
  // ── STEP 1: Scrape Amazon FRESH — always, no cache ────────────────────────
  // Clean the URL to bare dp/ASIN form for best results
  const cleanUrl = (() => {
    const m = sourceUrl.match(/\/dp\/([A-Z0-9]{10})/);
    return m ? `https://www.amazon.com/dp/${m[1]}?th=1` : sourceUrl;
  })();

  let product = null;
  product = await scrapeAmazonProduct(cleanUrl, null).catch(e => {
    console.warn('[revise] scrape failed:', e.message);
    return null;
  });

  // Fallback to cached combo data only if Amazon is completely unreachable
  if (!product) {
    const hasFallback = fallbackComboAsin && Object.keys(fallbackComboAsin).length > 0;
    if (hasFallback) {
      console.log('[revise] Amazon unreachable — using cached combo data for stock/price only');
      product = {
        price: parseFloat(fallbackPrice) || 0,
        inStock: fallbackInStock !== false,
        hasVariations: !!(fallbackVariations?.length),
        variations:      fallbackVariations      || [],
        comboAsin:       fallbackComboAsin       || {},
        comboInStock:    fallbackComboInStock     || {},
        comboPrices:     fallbackComboPrices      || {},
        sizePrices:      {},
        images: [], title: '', aspects: {}, bullets: [], descriptionPara: '',
        _primaryDimName:   fallbackPrimaryDimName   || null,
        _secondaryDimName: fallbackSecondaryDimName || null,
      };
    } else {
      return res.status(503).json({
        error: 'Amazon is blocking requests and no cached data available. Try again later.',
        skippable: true,
      });
    }
  }
  ...
  // Fuzzy-normalised lookup (handles name capitalisation/spacing differences)
  ...
  const lookupStock = (pv, sv) => {
    const e = `${pv||''}|${sv||''}`;
    if (comboInStock[e] !== undefined) return comboInStock[e];
    return _ciN[`${_nv(pv)}|${_nv(sv)}`];
  };
  const lookupPrice = (pv, sv) => {
    const e = `${pv||''}|${sv||''}`;
    if (comboPrices[e]) return comboPrices[e];
    if (_cpN[`${_nv(pv)}|${_nv(sv)}`]) return _cpN[`${_nv(pv)}|${_nv(sv)}`];
    const sizePrices = product.sizePrices || {};
    return sizePrices[sv||''] || sizePrices[pv||''] || basePrice || 0;
  };
  ...
  const getQty = (pv, sv) => {
    ...
    const inStock = lookupStock(pv, sv);
    // Only mark OOS if explicitly false — missing key = assume in stock
    return inStock === false ? 0 : defaultQty;
  };

  const getPrice = (pv, sv) => {
    const p = applyMk(lookupPrice(pv, sv));
    return p > 0 ? p : applyMk(basePrice) || 9.99;
  };

  console.log(`[revise] scraped: price=$${basePrice} inStock=${freshStock} combos=${Object.keys(comboAsin).length}`);

  const priceChanges = [], stockChanges = [];

  // ── STEP 2: Check if variation or simple ──────────────────────────────────
  ...
  console.log(`[revise] type=${isVariation ? 'variation' : 'simple'} variants=${variantSkus.length}`);

  // Enable OOS control BEFORE setting any qty=0
  if (ebayListingId) {
    await enableOutOfStockControl(access_token, ebayListingId).catch(() => {});
    await sleep(200);
  }

  if (isVariation) {
    // ── VARIATION: read current Color/Size aspects from eBay, update qty+price ──

    // Read existing aspects for all SKUs from eBay (to know which Color/Size each SKU is)
    const skuAspects = {};
    for (let i = 0; i < variantSkus.length; i += 20) {
      ...
      for (const item of (bd.inventoryItems || [])) {
        ...
        // Find primary (Color) and secondary (Size) values
        const pKeys = Object.keys(asp).filter(k => /color|colour/i.test(k));
        const sKeys = Object.keys(asp).filter(k => /size/i.test(k));
        const oKeys = Object.keys(asp).filter(k => !/color|colour|size/i.test(k));
        const pv = pKeys.length ? asp[pKeys[0]]?.[0] : (oKeys.length ? asp[oKeys[0]]?.[0] : null);
        const sv = sKeys.length ? asp[sKeys[0]]?.[0] : (pKeys.length && oKeys.length ? asp[oKeys[0]]?.[0] : null);
        skuAspects[item.sku] = {
          pv: pv || null,
          sv: sv || null,
          existingQty: item.inventoryItem?.availability?.shipToLocationAvailability?.quantity ?? -1,
        };
      }
      if (i + 20 < variantSkus.length) await sleep(80);
    }
    console.log(`[revise] read aspects: ${Object.keys(skuAspects).length}/${variantSkus.length} SKUs`);

    // Update qty for each variant — read existing item, only change availability
    let firstStatus = 0;
    for (let i = 0; i < variantSkus.length; i += 15) {
      await Promise.all(variantSkus.slice(i, i+15).map(async sku => {
        const { pv, sv, existingQty } = skuAspects[sku] || {};
        const newQty = getQty(pv, sv);

        // Only update if qty changed
        if (existingQty === newQty) return;

        // GET existing item to preserve all product data
        const existing = await fetch(`${EBAY_API}/sell/inventory/v1/inventory_item/${encodeURIComponent(sku)}`, { headers: auth })
          .then(r => r.ok ? r.json() : null).catch(() => null);
        if (!existing) return;
        ...
        if (r.status === 401 && firstStatus === 0) firstStatus = 401;
        if (r.ok || r.status === 204) {
          if (existingQty >= 0) {
            const change = `${sku.slice(-12)}: qty ${existingQty}→${newQty}`;
            stockChanges.push(change);
            console.log(`[revise/var] ${change}`);
          }
        }
      }));
    ...
    if (firstStatus === 401) {
      return res.status(401).json({ error: 'eBay token missing inventory permission. Re-authorize in Settings.', code: 'INVENTORY_401' });
    }

    // Update offer price for each variant — only if changed
    let priceUpdates = 0;
    for (let i = 0; i < variantSkus.length; i += 8) {
      await Promise.all(variantSkus.slice(i, i+8).map(async sku => {
        const { pv, sv } = skuAspects[sku] || {};
        const newPrice = getPrice(pv, sv);
        ...
        if (Math.abs(newPrice - oldPrice) <= 0.01) return;
        ...
        priceChanges.push(`${sku.slice(-12)}: $${oldPrice.toFixed(2)}→$${newPrice.toFixed(2)}`);
        priceUpdates++;
        console.log(`[revise/var] price ${sku.slice(-20)}: $${oldPrice.toFixed(2)}→$${newPrice.toFixed(2)}`);
      }));
      if (i + 8 < variantSkus.length) await sleep(80);
    }

    console.log(`[revise] done — ${stockChanges.length} stock changes, ${priceUpdates} price changes`);
    return res.json({
      success: true, type: 'variant',
      updatedVariants: variantSkus.length, pricesUpdated: priceUpdates,
      failed: 0, total: variantSkus.length,
      images: 0, price: applyMk(basePrice),
      inStock: freshStock, title: '',
    ...
    // ── SIMPLE: single inventory item — update qty + price only ──────────────
    const newQty   = freshStock ? defaultQty : 0;
    const newPrice = getPrice(null, null);

    // GET existing item to preserve all product data
    const existing = await fetch(`${EBAY_API}/sell/inventory/v1/inventory_item/${encodeURIComponent(ebaySku)}`, { headers: auth })
      .then(r => r.ok ? r.json() : null).catch(() => null);

    if (!existing) {
      return res.status(400).json({
        error: 'Inventory item not found — listing may have been ended. Use Re-push to recreate.',
        code: 'LISTING_NOT_FOUND',
      });
    }

    const oldQty = existing?.availability?.shipToLocationAvailability?.quantity ?? -1;

    // Only PUT if qty changed
    if (oldQty !== newQty) {
      ...
      if (!ir.ok) {
        ...
        if (t.includes('25710') || t.includes('not found') || t.includes('no longer')) {
          return res.status(400).json({ error: 'eBay listing no longer active. Use Re-push to recreate.', code: 'LISTING_NOT_FOUND' });
        }
        return res.status(400).json({ error: `Inventory update failed: ${t.slice(0,200)}` });
      }
      if (oldQty >= 0) stockChanges.push(`qty ${oldQty}→${newQty}`);
    }

    // Update offer price only if changed
    ...
    if (offer?.offerId) {
      ...
      if (Math.abs(newPrice - oldPrice) > 0.01) {
        ...
        console.log(`[revise/simple] price: $${oldPrice.toFixed(2)}→$${newPrice.toFixed(2)}`);
      }
    }

    console.log(`[revise/simple] price=$${newPrice} qty=${newQty}`);
    return res.json({
      success: true, type: 'simple',
      updatedVariants: 1, images: 0,
      price: newPrice, inStock: freshStock, title: '',
      priceChanges, stockChanges, imageChanges: [],
      comboInStock: product.comboInStock || null,
      comboAsin:    product.comboAsin    || null,
      comboPrices:  product.comboPrices  || null,
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'revise') {
      return handleRevise({ body, res, sleep, getEbayUrls });
    }

    // ── AUTH: generate eBay OAuth URL ─────────────────────────────────────────
    if (action === 'auth') {
      const { CLIENT_ID, EBAY_AUTH } = getEbayUrls();
      if (!CLIENT_ID) return res.status(500).json({ error: 'EBAY_CLIENT_ID not set in environment' });
      const redirect = `https://dropsync-one.vercel.app/api/ebay?action=callback`;
      const url = `${EBAY_AUTH}?client_id=${CLIENT_ID}&redirect_uri=${encodeURIComponent(redirect)}&response_type=code&scope=${encodeURIComponent(SCOPES)}&prompt=login`;
      return res.json({ url });
    }
    ...
    if (action === 'callback') {
      ...
      if (!code) return res.status(400).send('<script>window.close();</script><p>No code</p>');
      const { CLIENT_ID, CLIENT_SECRET, EBAY_TOK } = getEbayUrls();
      const redirect = `https://dropsync-one.vercel.app/api/ebay?action=callback`;
      const basic = Buffer.from(`${CLIENT_ID}:${CLIENT_SECRET}`).toString('base64');
      const tok = await fetch(EBAY_TOK, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded', Authorization: `Basic ${basic}` },
        body: `grant_type=authorization_code&code=${encodeURIComponent(code)}&redirect_uri=${encodeURIComponent(redirect)}`,
      }).then(r => r.json());
      if (!tok.access_token) {
        return res.status(500).send(`<script>window.opener?.postMessage({type:'ebay_auth',error:'${tok.error_description||'Token exchange failed'}'},'*');window.close();</script>`);
      }
      const expiry = Date.now() + (tok.expires_in || 7200) * 1000;
      const html = `<script>
        var d={type:'ebay_auth',token:'${tok.access_token}',refresh:'${tok.refresh_token||''}',expiry:${expiry}};
        try{window.opener.postMessage(d,'*');}catch(e){}
        try{localStorage.setItem('ebay_auth_pending',JSON.stringify(d));}catch(e){}
        document.write('<p>Connected! You can close this window.</p>');
        setTimeout(function(){window.close();},2000);
      </script><p>Connected to eBay! Closing...</p>`;
    ...
    if (action === 'refresh') {
      ...
      const tok = await fetch(EBAY_TOK, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded', Authorization: `Basic ${basic}` },
        body: `grant_type=refresh_token&refresh_token=${encodeURIComponent(refresh_token)}&scope=${encodeURIComponent(SCOPES)}`,
      }).then(r => r.json());
      if (!tok.access_token) return res.status(401).json({ error: tok.error_description || 'Refresh failed' });
      return res.json({ access_token: tok.access_token, expiry: Date.now() + (tok.expires_in || 7200) * 1000 });
    }

    // ── SCRAPE: scrape an Amazon product ─────────────────────────────────────
    if (action === 'scrape') {
...
<GetMyeBaySellingRequest xmlns="urn:ebay:apis:eBLBaseComponents">
  ...
  <ErrorLanguage>en_US</ErrorLanguage><WarningLevel>High</WarningLevel>
    ...
    // ── SYNC: price + stock ONLY — no images, no AI, no scraping ────────────────
    // Worker job: update offer price and inventory qty per variant using cached
    // combo data. Never touches title, description, images, or aspects.
    if (action === 'sync') {
      ...
      // ── Get price / stock data from cached fallback (no Amazon scrape) ──────
      const fallbackPrice        = parseFloat(body.fallbackPrice) || 0;
      const fallbackInStock      = body.fallbackInStock !== false;
      const comboAsin            = body.fallbackComboAsin    || {};
      const comboInStock         = body.fallbackComboInStock || {};
      const comboPrices          = body.fallbackComboPrices  || {};
      const sizePrices           = (body.fallbackVariations  || []).reduce((acc, v) => {
        for (const val of (v.values || [])) if (val.price) acc[val.value] = parseFloat(val.price);
        return acc;
      }, {});
      const basePrice            = parseFloat(fallbackPrice) || 0;
      const hasComboData         = Object.keys(comboInStock).length > 0 || Object.keys(comboPrices).length > 0;

      // Fuzzy normalised lookup (handles capitalisation/spacing differences)
      const _nv = v => (v || '').toLowerCase().replace(/\s+/g, ' ').trim();
      const _ciN = {}, _cpN = {};
      for (const [k, v] of Object.entries(comboInStock)) { const [p,s]=k.split('|'); _ciN[`${_nv(p)}|${_nv(s)}`]=v; }
      for (const [k, v] of Object.entries(comboPrices))  { const [p,s]=k.split('|'); _cpN[`${_nv(p)}|${_nv(s)}`]=v; }

      const lookupStock = (c, s) => {
        const e=`${c||''}|${s||''}`; if (comboInStock[e]!==undefined) return comboInStock[e];
        return _ciN[`${_nv(c)}|${_nv(s)}`];
      };
      const lookupPrice = (c, s) => {
        const e=`${c||''}|${s||''}`; if (comboPrices[e]) return comboPrices[e];
        if (_cpN[`${_nv(c)}|${_nv(s)}`]) return _cpN[`${_nv(c)}|${_nv(s)}`];
        return sizePrices[s||''] || sizePrices[c||''] || basePrice || 0;
      };

      const getQtySync = (c, s) => {
        if (!fallbackInStock) return 0;
        if (!hasComboData) return defaultQty;
        const inStock = lookupStock(c, s);
        return inStock === false ? 0 : defaultQty;
      };
      const getPriceSync = (c, s) => {
        const p = applyMk(lookupPrice(c, s));
        return p > 0 ? p : applyMk(basePrice) || 9.99;
      };

      const priceChanges = [], stockChanges = [];

      // ── Check if variation or simple ────────────────────────────────────────
      const grpRes     = await fetch(
        `${EBAY_API_S}/sell/inventory/v1/inventory_item_group/${encodeURIComponent(groupSkuSync)}`,
        { headers: auth }
      ).then(r => r.ok ? r.json() : null).catch(() => null);
      const variantSkusSync = grpRes?.variantSKUs || [];
      const isVarSync = variantSkusSync.length > 0;

      console.log(`[sync] sku=${groupSkuSync.slice(0,30)} type=${isVarSync?'variation':'simple'} markup=${markupPct}% hasCombo=${hasComboData}`);

      if (isVarSync) {
        // ── VARIATION: update only qty + price per variant ───────────────────
        // Read current Color/Size aspects for each SKU
        const skuAsp = {};
        for (let i = 0; i < variantSkusSync.length; i += 20) {
          const qs = variantSkusSync.slice(i, i+20).map(s => `sku=${encodeURIComponent(s)}`).join('&');
          const bd = await fetch(`${EBAY_API_S}/sell/inventory/v1/inventory_item?${qs}`, { headers: auth })
            .then(r => r.json()).catch(() => ({}));
          for (const item of (bd.inventoryItems || [])) {
            const asp = item.inventoryItem?.product?.aspects || {};
            // Find primary (Color) and secondary (Size) dimension values
            // Try common names first, then fall back to any aspect values
            const colorKeys = Object.keys(asp).filter(k => /color|colour/i.test(k));
            const sizeKeys  = Object.keys(asp).filter(k => /size/i.test(k));
            const otherKeys = Object.keys(asp).filter(k => !/color|colour|size/i.test(k));
            const color = colorKeys.length ? asp[colorKeys[0]]?.[0] : null;
            const size  = sizeKeys.length  ? asp[sizeKeys[0]]?.[0]  : (otherKeys.length ? asp[otherKeys[0]]?.[0] : null);
            skuAsp[item.sku] = { color, size, rawAspects: asp };
            skuAsp[item.sku].existingQty = item.inventoryItem?.availability?.shipToLocationAvailability?.quantity ?? -1;
          }
          if (i + 20 < variantSkusSync.length) await sleep(80);
        }

        // Update qty for each variant (price set per offer separately)
        const updates = variantSkusSync.map(sku => {
          const { color, size, existingQty, rawAspects } = skuAsp[sku] || {};
          const newQty = getQtySync(color, size);
          return { sku, color, size, newQty, existingQty, rawAspects };
        });

        // Batch update inventory item quantity only (preserve all other fields)
        let firstStatus = 0;
        for (let i = 0; i < updates.length; i += 15) {
          await Promise.all(updates.slice(i, i+15).map(async ({ sku, newQty, existingQty }) => {
            // Read existing item to preserve product data
            const existing = await fetch(`${EBAY_API_S}/sell/inventory/v1/inventory_item/${encodeURIComponent(sku)}`, { headers: auth })
              .then(r => r.ok ? r.json() : null).catch(() => null);
            if (!existing) return;

            const r = await fetch(`${EBAY_API_S}/sell/inventory/v1/inventory_item/${encodeURIComponent(sku)}`, {
              method: 'PUT', headers: auth,
              body: JSON.stringify({
                ...existing,
                // Only change availability — preserve product (title, images, aspects, description)
                availability: { shipToLocationAvailability: { quantity: newQty } },
              }),
            });
            if (r.status === 401 && firstStatus === 0) firstStatus = 401;

            if (existingQty >= 0 && existingQty !== newQty) {
              stockChanges.push(`${sku.slice(-12)}: qty ${existingQty}→${newQty}`);
              console.log(`[sync/var] qty ${existingQty}→${newQty} for ${sku.slice(-20)}`);
            }
          }));
          if (i + 15 < updates.length) await sleep(100);
        }
        ...
        // Update offer price per variant SKU
        let priceUpdates = 0;
        for (let i = 0; i < updates.length; i += 8) {
          await Promise.all(updates.slice(i, i+8).map(async ({ sku, color, size }) => {
            const newPrice = getPriceSync(color, size);
            const ol = await fetch(`${EBAY_API_S}/sell/inventory/v1/offer?sku=${encodeURIComponent(sku)}`, { headers: auth })
              .then(r => r.json()).catch(() => ({}));
            const offer = (ol.offers || [])[0];
            if (!offer?.offerId) return;
            const oldPrice = parseFloat(offer.pricingSummary?.price?.value || 0);
            if (Math.abs(newPrice - oldPrice) > 0.01) {
              await fetch(`${EBAY_API_S}/sell/inventory/v1/offer/${offer.offerId}`, {
                method: 'PUT', headers: auth,
                body: JSON.stringify({ pricingSummary: { price: { value: newPrice.toFixed(2), currency: 'USD' } } }),
              }).catch(() => {});
              priceChanges.push(`${sku.slice(-12)}: $${oldPrice.toFixed(2)}→$${newPrice.toFixed(2)}`);
              priceUpdates++;
              console.log(`[sync/var] price $${oldPrice.toFixed(2)}→$${newPrice.toFixed(2)} for ${sku.slice(-20)}`);
            }
          }));
          if (i + 8 < updates.length) await sleep(80);
        }

        console.log(`[sync/var] done — ${stockChanges.length} stock changes, ${priceUpdates} price changes`);
        return res.json({
          success: true, type: 'variant',
          updatedVariants: variantSkusSync.length,
          images: 0,  // worker never touches images
        ...
        // ── SIMPLE listing: update qty + price only ──────────────────────────
        const freshStock = fallbackInStock;
        const newPrice   = getPriceSync(null, null);
        const newQty     = freshStock ? defaultQty : 0;

        // Read existing item to preserve all product data
        const existingItem = await fetch(`${EBAY_API_S}/sell/inventory/v1/inventory_item/${encodeURIComponent(groupSkuSync)}`, { headers: auth })
          .then(r => r.ok ? r.json() : null).catch(() => null);

        if (!existingItem) {
          return res.status(400).json({ error: 'Inventory item not found — may have been deleted. Use Re-push to recreate.' });
        }

        const oldQty = existingItem?.availability?.shipToLocationAvailability?.quantity ?? -1;

        const ir = await fetch(`${EBAY_API_S}/sell/inventory/v1/inventory_item/${encodeURIComponent(groupSkuSync)}`, {
          method: 'PUT', headers: auth,
          body: JSON.stringify({
            ...existingItem,
            // Only change qty — preserve product (title, images, aspects, description)
            availability: { shipToLocationAvailability: { quantity: newQty } },
          }),
        });
        ...
        // Update offer price
        const ol = await fetch(`${EBAY_API_S}/sell/inventory/v1/offer?sku=${encodeURIComponent(groupSkuSync)}`, { headers: auth })
          .then(r => r.json()).catch(() => ({}));
        const offer = (ol.offers || [])[0];
        if (offer?.offerId) {
          const oldPrice = parseFloat(offer.pricingSummary?.price?.value || 0);
        ...
        console.log(`[sync/simple] price=$${newPrice} qty=${newQty}`);
        return res.json({
          success: true, type: 'simple',
          updatedVariants: 1,
          images: 0,  // worker never touches images
...
Return ONLY the optimized title text, nothing else. No quotes, no explanation.` }]
    ...
    if (action === 'replenish') {
      ...
      // ── STEP 2: Wipe eBay listing content (clear images/desc — keep qty ≥ 1) ─
      // NEVER set quantity to 0 — eBay immediately ends a GTC listing when qty=0.
      // Keep qty at its current value (or at least 1) during the wipe window.
      // handleRevise (called in Step 5) will set the correct quantity per variant.
      try {
        if (snapshot?.type === 'variation' && variantSkusR.length) {
          for (let i = 0; i < variantSkusR.length; i += 15) {
            await Promise.all(variantSkusR.slice(i, i+15).map(async sku => {
              ...
              const currentQty = existing.availability?.shipToLocationAvailability?.quantity ?? 1;
              await fetch(`${EBAY_API_R}/sell/inventory/v1/inventory_item/${encodeURIComponent(sku)}`, {
                method: 'PUT', headers: authR,
                // Keep existing data intact — only clear description; qty stays ≥ 1
                body: JSON.stringify({
                  ...existing,
                  availability: { shipToLocationAvailability: { quantity: Math.max(currentQty, 1) } },
                }),
              }).catch(() => {});
            }));
          }
          ...
          await fetch(`${EBAY_API_R}/sell/inventory/v1/inventory_item/${encodeURIComponent(ebaySku)}`, {
            ...
            body: JSON.stringify({
              ...existing,
              product: { ...existing.product, title: existing.product?.title || 'Replenishing…' },
              availability: { shipToLocationAvailability: { quantity: Math.max(currentQty, 1) } },
            }),
          }).catch(() => {});
        }
        console.log('[replenish] cleared content (listing kept alive with qty≥1)');
      ...
      await handleRevise({ body: revisedBody, res: fakeRes, sleep, getEbayUrls });
